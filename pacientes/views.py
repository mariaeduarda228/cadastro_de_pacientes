...
def pacientes(request):
    if request.method == "GET":
        ...
        return render(
            request,
            "pacientes.html",
            {"pacientes": pacientes, "queixas": Pacientes.queixas_choices},
        )
...
def paciente_view(request, id):
    ...
    if request.method == "GET":
        ...
        tuple_grafico = (
            [str(consulta.data) for consulta in consultas],
            [str(consulta.humor) for consulta in consultas],
        )
        # consultas_list = []
        ...
        total_visualizacoes = Visualizacoes.objects.filter(
        ...
        consulta.save()
        messages.add_message(
            request, constants.SUCCESS, "Consulta cadastrada com sucesso"
        )
        return redirect("paciente_view", id=id)
